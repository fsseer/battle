...
export default function Battle() {
	...
	// Pixi.js canvas refs (reserved for future; not used in DOM placeholder)
			...
			<div className="panel">
				...
				<div className="parchment" style={{ marginTop: 8 }}>
					...
					>
						{/* 모래바닥 */}
						<div
							style={{ position: 'absolute', left: cam.x + 0, top: cam.y + 150, width: 360, height: 50, background: '#e5d3a1' }}
						/>
						{/* 전투원 */}
						<img src="/sprites/fighter_red.svg" style={{ position: 'absolute', left: cam.x + 64, top: cam.y + 88, width: 64, height: 64 }} />
						<img src="/sprites/fighter_blue.svg" style={{ position: 'absolute', left: cam.x + 260, top: cam.y + 88, width: 64, height: 64 }} />
						{/* 간단한 효과 */}
						{choice === 'slash' && (
							<div style={{ position: 'absolute', left: cam.x + 120, top: cam.y + 110, width: 50, height: 3, background: '#aa0000', transform: 'rotate(-20deg)' }} />
						)}
						{choice === 'feint' && (
							<div style={{ position: 'absolute', left: cam.x + 120, top: cam.y + 110, width: 40, height: 3, background: '#aa8800' }} />
						)}
						{choice === 'block' && (
							<div style={{ position: 'absolute', left: cam.x + 230, top: cam.y + 90, width: 20, height: 40, background: '#888' }} />
						)}
						{choice === 'parry' && (
							<img src="/sprites/spark.svg" style={{ position: 'absolute', left: cam.x + 240, top: cam.y + 110, width: 24, height: 24, filter: 'hue-rotate(120deg)' }} />
						)}
						{spark && spark.t > 0 && (
							<img src="/sprites/spark.svg" style={{ position: 'absolute', left: cam.x + spark.x, top: cam.y + spark.y, width: 14, height: 14, opacity: Math.max(0, spark.t / 200) }} />
						)}
						{particles.map((p, idx) => (
							<img key={idx} src="/sprites/spark.svg" style={{ position: 'absolute', left: cam.x + p.x, top: cam.y + p.y, width: 6, height: 6, opacity: Math.max(0.1, p.life / p.ttl) }} />
						))}
						{trail && trail.t > 0 && (
							<div style={{ position: 'absolute', left: cam.x + 120, top: cam.y + 105, width: 60, height: 4, background: '#ffaa66', opacity: Math.max(0, trail.t / trail.ttl) }} />
						)}
					</div>
