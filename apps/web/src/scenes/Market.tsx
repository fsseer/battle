...
export default function Market() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [cat, inv] = await Promise.all([getShopCatalog('market'), getInventory()])
    ...
    <div className="lobby-layout">
      ...
      <div className="center-content-wrapper">
        <div className="center-background-area">
          ...
          {gold !== null && (
            <div style={{ color: '#ffd700' }}>보유 골드: {gold}</div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div>
              ...
              {catalog.map((it) => (
                <div key={it.id} style={{ background: 'rgba(0,0,0,0.5)', padding: 12, borderRadius: 8, marginBottom: 8 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <button className="menu-icon-btn" onClick={() => handleBuy(it.id)}>구매</button>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div>
              ...
              {inventory.length === 0 && <div style={{ color: '#ccc' }}>보유 소모품이 없습니다.</div>}
              {inventory.map((it) => (
                <div key={it.itemId} style={{ background: 'rgba(0,0,0,0.5)', padding: 12, borderRadius: 8, marginBottom: 8 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ color: '#fff', fontWeight: 600 }}>{it.name} x{it.quantity}</div>
                      <div style={{ color: '#ccc', fontSize: 12 }}>{it.description}</div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <span style={{ color: '#9acd32' }}>{it.sellPrice} G</span>
                      <button className="menu-icon-btn" onClick={() => handleSell(it.itemId)}>1개 판매</button>
                    </div>
                  </div>
                </div>
              ))}
