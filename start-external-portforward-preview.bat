...
rem External access via port-forwarding (router) — PREVIEW mode
rem - Builds web and serves static assets via Vite preview (faster first byte than dev)
rem - API runs in dev (tsx) as before
...
echo [web] Build with VITE_SERVER_ORIGIN=%VITE_SERVER_ORIGIN%
cmd /c "set VITE_SERVER_ORIGIN=%VITE_SERVER_ORIGIN%&& npm run build"
echo [web] Preview on 0.0.0.0:5173 (static)
start "battle-web-dev" cmd /k "set VITE_SERVER_ORIGIN=%VITE_SERVER_ORIGIN% && npm run preview -- --host 0.0.0.0 --port 5173"
popd
